...
const Projects = () => {
    ...
    const CategorizedProjs = projs.reduce((acc, proj) => {
      ...
      const setEditForm = (id,projname, url, categ, devperiod) => {
        ...
        document.getElementById('selecteduploadimg').src = document.querySelector('#proj_' + id + ' .itemimg').src;
      ...
      document.addEventListener("click", (event) => {
        ...
        var isClickInsidePopupButton = event.target.classList.contains("admintoggle");
        if (!isClickInsidePopupButton) {
            ...
            popups.forEach((popup) => {
                ...
                var toggleBtn = document.querySelector(`[data-popup-id="${popupId.substring(11)}"]`);
                toggleBtn.setAttribute("aria-expanded", "false");
            });
        }
      ...
      return (<div id="projects">
        ...
        {Object.entries(CategorizedProjs).map(([categ, ProjInCateg]) => (
            <div class='container' key={categ} /*style={{backgroundColor:'rgba('+themecolor+',0.60)'}}*/>
            ...
            <div class='cont-grid4'>
                {ProjInCateg.map((val) =>{ return (
                    <div class='grid-content' key={val._id} id={"proj_" + val._id}>
                    <a href={val.url} target="_blank">
                        <img src={cloudstorage+'/data/projects/'+val.mainphoto} className='itemimg' />
